...
raw_dir = r"/data/wwwroot/python/pinyin/raw/result/raw"
...
def fetch_pinyin(words, sep="'"):
    ...
    url = "https://zhongwenzhuanpinyin.bmcx.com/web_system/bmcx_com_www/system/file/zhongwenzhuanpinyin/data_v2/?ajaxtimestamp=1763098715995"
    headers = {
        "Accept": "*/*",
    ...
    pinyins = response.json()

    print("pinyins are: ", json.dumps(pinyins, ensure_ascii=False))
    ...
    for pinyin in pinyins:
        if pinyin.get("字符") != ",":
            current_word = current_word + pinyin.get("字符")
            current_pinyin.append(pinyin.get("拼音")[0])
        else:
            pinyin_words.append([current_word, current_pinyin])
            current_word = ""
            current_pinyin = []
    ...
    for i in range(len(words)):
        l = len(words[i])
        chars = pinyin_words[i][0]
        pinyin = sep.join(pinyin_words[i][1])
        if l == 1:
            freq = None
            other = None
        elif 1 < l <= 3:
