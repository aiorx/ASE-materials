...
def __init__(
    ...
    with tf.name_scope('output_layer'):
        ...
        w_out = tf.Variable(tf.random_uniform([fc_layers[-1], num_classes], minval=-0.1, maxval=0.1), name='w')
        b_out = tf.Variable(tf.random_uniform(shape=[num_classes], minval=-0.1, maxval=0.1), name='b')
        self.scores = tf.nn.xw_plus_b(self.x_flat, w_out, b_out, name="scores")
