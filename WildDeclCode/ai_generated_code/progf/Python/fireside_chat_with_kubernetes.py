...
def interactive_kubernetes_assistant():
    ...
    def execute_kubectl_command(kubectl_args, description):
        ...
        try:
            ...
            confirmation = input("🔐 Do you want to execute this command? [Y/n] (Enter = Yes): ").strip().lower()
    ...
    # Assisted using common GitHub development utilities - Initialize conversation history
    ...
    while True:
        try:
            user_input = input("\n🤔 What would you like to know about your Kubernetes cluster? ")
            ...
            if message.tool_calls:
                print(f"\n🔧 Running kubectl commands... ({len(message.tool_calls)} function{'s' if len(message.tool_calls) > 1 else ''} to execute)")
                ...
                if confirmation_required and len(message.tool_calls) > 1:
                    print("⚠️  Note: One or more commands require your confirmation before execution.")
                ...
                for i, tool_call in enumerate(message.tool_calls):
                    ...
                    if len(message.tool_calls) > 1:
                        print(f"\n📋 Executing function {i+1}/{len(message.tool_calls)}: {function_name}")
                    ...
                    if function_name in function_map:
                        ...
                        if "Command execution denied by user" in output:
                            print(f"⏭️  Skipping denied command, continuing with remaining functions...")
                    else:
                ...
                print(f"\n✅ Completed {len(message.tool_calls)} function call{'s' if len(message.tool_calls) > 1 else ''}")
