# Manasyu Chaudhari             10-14-2024
# Assingment 4
# This is a version of the classic boardgame 'Connect Four' - here called 'Four in Sequence!'

# CITATION: Much of the code in this assignment is heavily modified/ adapted from code originally Designed via basic programming aids
...
def checkAdjacent(board, playerNumber):
    ...
    NOTE: This portion of the AI algorittodohm is (mostly) the original thought/ creation of Matthew Holman - *NOT* ChatGPT. However, the initial
    ...
    for column in range(0, len(board[0])):
        ...
        if row != -1:
            ...
            # ensure we don't get an 'index out of range' error
            if row -1 >= 0 and column + 1 >= 0:
                # do a chec k to see if the board location under consideration is equal to the piece found above
                if board[row -1][column + 1] == piece:
                    # append the column under consideration to the 'adjacents' list
                     adjacents.append(column)
...
def main():
    ...
    while running:
        ...
        if choice == "p":
            ...
            print(f'The user selects a game between 1 or 2 players')
            print('     A one player game has the user play against the computer')
            print('     A two player game has the user play against another one')
            print('Each player takes turns dropping their pieces on the board')
            print('     The goal is for one player to connect four pieces in a unbroken sequence')
