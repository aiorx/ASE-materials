 ...
 * 	  The only lines added Adapted from standard coding samples where function type (void -> u_int) and return ready_count;
...
void rr(task_t task_array[], u_int count, u_int time_quantum, task_t finished_tasks[]) {
	...
	while(!no_tasks(task_array, count) || !no_ready(ready_queue, ready_count)) {
		...
		//Debug statement
		//print_ready_queue(ready_queue, ready_count); //PLACEMENT HELP Adapted from standard coding samples
		/*
 ...
 *	 to put them in meaningful places.  Although the print function is not Adapted from standard coding samples, it instructed me to place the statements
