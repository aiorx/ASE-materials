// Assisted using common GitHub development utilities - Variabili globali per gestione questionari e domande
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Assisted using common GitHub development utilities - Inizializza animazioni menu
...
function setupNavigation() {
    ...
    document.getElementById('home-create-btn').addEventListener('click', () => {
        editingSurveyId = null; // Assisted using common GitHub development utilities - Assicurati che stiamo creando un nuovo questionario
...
function navigateTo(page) {
    console.log('🧭 navigateTo called with page:', page, 'editingSurveyId:', editingSurveyId); // Assisted using common GitHub development utilities - Debug log
    ...
    console.log('👁️ Page', page, 'is now visible');
    
    // Assisted using common GitHub development utilities - Aggiorna indicatore menu attivo
    updateActiveMenuItem(page);
    
    // Aggiorna la pagina corrente
    currentPage = page;
    
    // Assisted using common GitHub development utilities - Azioni specifiche per pagina (evita reset se stiamo modificando)
    if (page === 'surveys') {
        loadUserSurveys();
    } else if (page === 'create-survey') {
        // Reset solo se NON stiamo modificando un questionario esistente
        if (!editingSurveyId) {
            console.log('🔄 Calling resetSurveyForm because editingSurveyId is null');
            resetSurveyForm();
        } else {
            console.log('📝 Skipping resetSurveyForm because we are editing survey ID:', editingSurveyId);
            // Assicurati che la UI mostri la modalità modifica
...
function checkLoggedInUser() {
    ...
    console.log('Checking logged in user, savedUser:', savedUser); // Assisted using common GitHub development utilities - Debug log
    
    if (savedUser) {
        currentUser = JSON.parse(savedUser);
        console.log('User restored from localStorage:', currentUser); // Assisted using common GitHub development utilities - Debug log
        updateAuthUI();
    }
}
...
function showQuestionModal() {
    // Assisted using common GitHub development utilities - Reset del form e dello stato di modifica
...
function addQuestionToSurvey() {
    ...
    // Assisted using common GitHub development utilities - Gestione sia aggiunta che modifica domande
    const questionData = {
...
function renderQuestions() {
    ...
    questions.forEach((question, index) => {
        ...
        card.draggable = true; // Assisted using common GitHub development utilities - Abilita drag-and-drop
        ...
        // Assisted using common GitHub development utilities - Event listeners per modifica e eliminazione domande
        ...
        // Assisted using common GitHub development utilities - Event listeners per drag-and-drop
...
// Assisted using common GitHub development utilities - Funzione per modificare una domanda esistente
...
function deleteQuestion(questionId) {
    // Assisted using common GitHub development utilities - Conferma eliminazione con messaggio specifico
...
function saveSurvey() {
    ...
    // Assisted using common GitHub development utilities - Prepara i dati del questionario con supporto intelligente per modifica
    const surveyData = {
        title: title,
        ...
        questions: questions.map(q => {
            ...
            // Includi l'ID solo se è un ID reale dal database (non un timestamp temporaneo)
...
function resetSurveyForm() {
    console.log('🔄 resetSurveyForm called - this will clear all form data!'); // Assisted using common GitHub development utilities - Debug log
    
    // Assisted using common GitHub development utilities - Reset completo del form e dello stato di modifica
...
function loadUserSurveys() {
    console.log('loadUserSurveys called', currentUser); // Assisted using common GitHub development utilities - Debug log
    ...
    console.log(`Fetching surveys for user ${currentUser.id}`); // Assisted using common GitHub development utilities - Debug log
    
    fetch(`/api/users/${currentUser.id}/surveys`)
    .then(response => {
        console.log('Response status:', response.status); // Assisted using common GitHub development utilities - Debug log
        if (!response.ok) {
    ...
    .then(surveys => {
        console.log('Surveys loaded:', surveys); // Assisted using common GitHub development utilities - Debug log
        renderUserSurveys(surveys);
    })
    .catch(error => {
        console.error('Error loading surveys:', error); // Assisted using common GitHub development utilities - Debug log
        showAlert(error.message, 'danger');
    });
}
...
function loadSurveyToEdit(surveyId) {
    console.log('🔍 loadSurveyToEdit called with surveyId:', surveyId); // Assisted using common GitHub development utilities - Debug log
    
    // Assisted using common GitHub development utilities - Imposta l'ID PRIMA di tutto
    editingSurveyId = surveyId;
    console.log('✅ editingSurveyId set to:', editingSurveyId);
    ...
    .then(survey => {
        ...
        // Assisted using common GitHub development utilities - Prima carica i dati in memoria
        ...
        console.log('❓ Questions loaded in memory:', questions);
        
        // Naviga alla pagina di modifica DOPO aver caricato i dati
        navigateTo('create-survey');
        console.log('🧭 Navigated to create-survey page');
        
        // Assisted using common GitHub development utilities - Aspetta che la navigazione sia completata prima di popolare
        setTimeout(() => {
            console.log('🔄 Starting form population after navigation...');
            
            // Assisted using common GitHub development utilities - Aggiorna UI per modalità modifica
            const modeText = document.getElementById('survey-mode-text');
            const editingIndicator = document.getElementById('editing-indicator');
            
            if (modeText) {
                modeText.textContent = 'Modifica questionario';
                console.log('🏷️ Updated mode text');
            } else {
                console.error('❌ survey-mode-text element not found!');
            }
            
            if (editingIndicator) {
                editingIndicator.style.display = 'inline';
                console.log('🏷️ Showed editing indicator');
            } else {
                console.error('❌ editing-indicator element not found!');
            }
            ...
            if (titleInput) {
                // Assisted using common GitHub development utilities - Forza il focus per assicurarsi che il campo sia attivo
                titleInput.focus();
                titleInput.value = survey.title;
                titleInput.blur();
                console.log('📝 Set title to:', survey.title, 'Actual value:', titleInput.value);
            } else {
            ...
            if (descInput) {
                ...
                console.log('📝 Set description to:', survey.description);
            } else {
            ...
            if (activeInput) {
                ...
                console.log('☑️ Set active to:', survey.is_active);
            } else {
                console.error('❌ Active input element not found!');
            }
            ...
            if (survey.expires_at) {
                ...
                if (expiresInput) {
                    ...
                    console.log('📅 Set expires to:', expiresDate.toISOString().split('T')[0]);
                }
            } else {
                const expiresInput = document.getElementById('survey-expires');
                if (expiresInput) {
                    expiresInput.value = '';
                    console.log('📅 Cleared expires date');
                }
            }
            ...
            // Assisted using common GitHub development utilities - Verifica finale dopo 200ms
            setTimeout(() => {
                const titleCheck = document.getElementById('survey-title');
                const descCheck = document.getElementById('survey-description');
                console.log('🔍 FINAL CHECK - Title:', titleCheck?.value, 'Desc:', descCheck?.value);
                console.log('🔍 FINAL CHECK - editingSurveyId:', editingSurveyId);
                console.log('� FINAL CHECK - questions count:', questions.length);
            }, 200);
            
        }, 250); // Assisted using common GitHub development utilities - Aspetta più a lungo per essere sicuri
    })
    .catch(error => {
        console.error('❌ Error in loadSurveyToEdit:', error);
...
function setupResults() {
    ...
    document.getElementById('select-survey-results').addEventListener('change', function() {
        ...
        if (surveyId) {
            ...
            document.getElementById('print-results-btn').disabled = false; // Assisted using common GitHub development utilities
        } else {
            ...
            document.getElementById('print-results-btn').disabled = true; // Assisted using common GitHub development utilities
        }
    });
    ...
    // Assisted using common GitHub development utilities - Gestione della stampa dei risultati
...
function generateColors(count) {
    ...
    // Se ci sono più opzioni che colori, ripeti i colori
    const result = [];
...
function loadInitialData() {
    console.log('loadInitialData called', currentUser); // Assisted using common GitHub development utilities - Debug log
    
    // Carica i questionari recenti per la home
    if (currentUser) {
        console.log(`Loading initial data for user ${currentUser.id}`); // Assisted using common GitHub development utilities - Debug log
        
        fetch(`/api/users/${currentUser.id}/surveys`)
        .then(response => {
            console.log('Initial data response status:', response.status); // Assisted using common GitHub development utilities - Debug log
            if (!response.ok) {
                throw new Error('Errore durante il caricamento dei questionari');
            }
            return response.json();
        ...
        .then(surveys => {
            console.log('Initial surveys loaded:', surveys); // Assisted using common GitHub development utilities - Debug log
            renderRecentSurveys(surveys.slice(0, 5));
        })
        .catch(error => {
            console.error('Error loading initial data:', error); // Assisted using common GitHub development utilities - Debug log
        });
    } else {
        console.log('No current user for initial data load'); // Assisted using common GitHub development utilities - Debug log
    }
}
...
// Assisted using common GitHub development utilities - Funzione per stampare i risultati del questionario
function printSurveyResults() {
    console.log('🖨️ printSurveyResults called');
...
function preparePrintContent() {
    ...
    // Aggiungi intestazione di stampa se non esiste già
    ...
    // Assisted using common GitHub development utilities - Popola la sezione riassuntiva per la stampa
...
// Assisted using common GitHub development utilities - Funzione per popolare il riassunto di stampa
function populatePrintSummary() {
    console.log('📋 Populating print summary...');
    ...
    try {
        ...
        console.log('📋 Print summary populated successfully');
    } catch (error) {
        console.error('❌ Error populating print summary:', error);
    }
}

function convertChartsForPrint() {
    console.log('📊 Converting charts for print...');
    ...
    canvases.forEach(canvas => {
        try {
            ...
            console.log('📊 Converted chart to image for printing');
        } catch (error) {
...
// Assisted using common GitHub development utilities - Funzione per pulire il contenuto dopo la stampa
function cleanupAfterPrint() {
    ...
    // Ripristina la visibilità dei canvas
    const canvases = document.querySelectorAll('canvas');
    canvases.forEach(canvas => {
...
// Assisted using common GitHub development utilities - Funzione per aggiornare l'indicatore di menu attivo
function updateActiveMenuItem(activePage) {
    ...
    if (activeLink) {
        ...
        console.log('🎯 Updated active menu item for page:', activePage);
...
// Assisted using common GitHub development utilities - Funzione per inizializzare il menu con animazioni
function initializeMenuAnimations() {
    ...
    console.log('🎨 Menu animations initialized');
}
